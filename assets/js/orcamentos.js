...
function limparFormulario() {
  ...
  if (sel) {
    ...
    let idxAbrir = Array.from(sel.options).findIndex(
      (o) => o.value === "abrindo"
    );
    if (idxAbrir === -1) {
      sel.add(new Option("Aberto", "abrindo"));
      idxAbrir = Array.from(sel.options).findIndex(
        (o) => o.value === "abrindo"
      );
    }
...
// abre modal com dados existentes de um card (status clicado)
function abrirOrcamentoExistente(card) {
  ...
  const cliente = fromDataset(
    "cliente",
    card.querySelector("li:nth-child(2) p")?.textContent?.trim() || ""
  );
  const endereco = fromDataset("endereco");
  const telefone = fromDataset("telefone");
  const dataOrcamento = fromDataset("dataOrcamento");
  const material = fromDataset(
    "material",
    card.querySelector("li:nth-child(3) p")?.textContent?.trim() || ""
  );
  ...
  const descricao = fromDataset(
    "descricao",
    card.querySelector(".descricao-limitada")?.textContent?.trim() || ""
  );
  const dataEntrega = fromDataset(
    "dataEntrega",
    card.querySelector("li:nth-child(1) p")?.textContent?.trim() || ""
  );
  const entrada = fromDataset("entrada");
  const restante = fromDataset("restante");
  const valor = fromDataset(
    "valor",
    card.querySelector("li:nth-child(5) p")?.textContent?.trim() || ""
  );

  const statusEl = card.querySelector(".tag-orcamento");
  const statusClass = statusEl
    ? Array.from(statusEl.classList).find((c) =>
        ["aberto", "confirmado", "arquivado"].includes(c)
      )
    : "";
...
function coletarDadosFormulario() {
  const s = getStatusSelect();
  const statusClass = s
    ? Array.from(s.classList).find((c) => c && c !== "status")
    : "";
  ...
  const valor = document
    ...
    .textContent.replace("Valor Total: ", "")
    .replace(",", ".");
  ...
  items.forEach((item) => {
    item.addEventListener("click", () => {
      ...
      const cards = document.querySelectorAll(
        ".container-orcamento .orcamento-display .card-orcamento"
      );
...
function TotalizarValores() {
  const totalEl =
    document.querySelector(".criarOrcamento .botoes .total") ||
...
function toggleDescricao(id, btn) {
  ...
  btn.textContent = descricao.classList.contains("expanded")
    ? "Ver menos"
    : "Ver mais";
}
